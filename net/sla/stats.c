...
static inline bool sla_pkt_consolidate(struct sla_interface_stats *stats)
{
    u32 prev;
    struct sla_stats_entry *stats_entry;
    struct sla_stats_entry *prev_stats_entry;
    ...
    stats_entry = &stats->entries[stats->cur_idx];
    prev_stats_entry = &stats->entries[prev];

    if(prev_stats_entry->discard)
        return false;

    if(prev_stats_entry->port != stats_entry->port)
        return false;

    if((prev_stats_entry->timestamp + config_data.limit_time) < stats_entry->timestamp)
        return false;

    if((prev_stats_entry->size + stats_entry->size) > config_data.max_size)
        return false;

    prev_stats_entry->size += stats_entry->size;
                                ...
                                const struct nf_hook_state *state)
{
    ...
    ktime_t tm = ktime_get_real();
    struct uplink_pkt *pkt;
    bool consolidated = false;
    struct sla_interface* iface = NULL;
    unsigned long flags;
    u64 timestamp = ktime_to_ms(tm); //tm.tv_sec*1000 + tm.tv_nsec/1000000;
...
{
    int err = 0;
    printk("#### %s in version V 4.4.4 \n", __func__);

    err = register_pernet_subsys(&sla_net_ops);

    if (err < 0)
    {
         printk(KERN_ERR "%s: register_pernet_subsys: error %d\n", err);
         return err;
    }
