...
static int cam_ois_get_dt_data(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	for (i = 0; i < soc_info->num_clk; i++) {
		soc_info->clk[i] = devm_clk_get(soc_info->dev,
					soc_info->clk_name[i]);
		if (!soc_info->clk[i]) {
			pr_err("Randy debug OIS " "get failed for %s",
				 soc_info->clk_name[i]);
			rc = -ENOENT;
			return rc;
		}
		CAM_DBG(CAM_OIS, "get for clk %s",
			soc_info->rgltr_name[i]);
	}
	rc = cam_ois_get_dt_i2c_info(o_ctrl);
	if(rc < 0)
	{
		pr_err("Randy debug OIS " "Get i2c info failed!");
		return -EINVAL;
	}
	...
	{
		pr_err("Randy debug OIS " "Get power setting failed!");
		return -EINVAL;
	}
