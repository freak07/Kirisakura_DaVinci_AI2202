...
int msm_vidc_add_session(struct msm_vidc_inst *inst)
{
	...
	if (count < core->capabilities[MAX_SESSION_COUNT].value) {
		...
		//add by vector +++
		dprintk_inst(VIDC_PRINTK, "debug", inst, "%s: add session %#x\n",
				__func__, inst->session_id);
		//add by vector ---
	} else {
		...
		list_for_each_entry(i, &core->instances, list){
			sprintf(id_str,"%s %#x",id_str,i->session_id);
		}
		i_vpr_e(inst, "%s: max limit %d already running %d sessions: %s\n",
			__func__, core->capabilities[MAX_SESSION_COUNT].value, count, id_str);
		//add by vector ---
		rc = -EINVAL;
	}
	core_unlock(core, __func__);
...
int msm_vidc_remove_session(struct msm_vidc_inst *inst)
{
	...
	list_for_each_entry_safe(i, temp, &core->instances, list) {
		if (i->session_id == inst->session_id) {
			...
			//add by vector +++
			dprintk_inst(VIDC_PRINTK, "debug", inst, "%s: removed session %#x\n",
				__func__, i->session_id);
			//add by vector ---
		}
	}
