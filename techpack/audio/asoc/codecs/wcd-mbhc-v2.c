	...
	if (!insertion) {
		...
		if (mbhc->impedance_detect &&
			...
			if ((mbhc->zl > mbhc->mbhc_cfg->linein_th) &&
				...
				pr_err("%s: Marking jack type as SND_JACK_LINEOUT\n",
				__func__);
			}
		}
...
void wcd_mbhc_elec_hs_report_unplug(struct wcd_mbhc *mbhc)
{
	...
	pr_err("%s: Report extension cable\n", __func__);
...
static void wcd_btn_lpress_fn(struct work_struct *work)
{
	...
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET) {
		pr_err("%s: Reporting long button press event, btn_result: %d\n",
			 __func__, btn_result);
...
static int wcd_mbhc_initialise(struct wcd_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_cfg->enable_usbc_analog) {
		...
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_INSREM_DBNC, 0xB); /* ASUS_BSP change to 512ms for ASUS HW design */
	}
