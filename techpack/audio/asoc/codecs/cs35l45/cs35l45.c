...
#if defined(REGISTER_DUMP_ENABLE)
static int cs35l45_amp_register_dump_get(struct snd_kcontrol *kcontrol,
...
static int cs35l45_amp_register_dump_put(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
	...
	struct cs35l45_private *cs35l45 =
		  snd_soc_component_get_drvdata(component);
	struct regmap *map = cs35l45->regmap;
	unsigned int val;
	...
	if (ucontrol->value.integer.value[0]) {
	  ret = regmap_get_max_register(map);
	  dev_err(cs35l45->dev, "%s: Max register = %d\n", __func__, ret);
	  if (0 >= ret)
		  return -EINVAL;
	  ...
	  for (i = 0; reg_max >= i; i += reg_stride) {
		  ...
		  if (ret) {
			  ...
			  dev_err(cs35l45->dev, "%s: Failed to read reg = 0x%08x, ret = %d\n", __func__, i, ret);
			  return ret;
		  } else {
			  dev_err(cs35l45->dev, "%07x: %08x\n", i, val);
		  }
	  }
	  regcache_cache_bypass(map, false);
	}
...
#if defined(REGISTER_DUMP_ENABLE)
	SOC_SINGLE_EXT("AMP Register Dump", SND_SOC_NOPM, 0, 2, 0,
			   cs35l45_amp_register_dump_get, cs35l45_amp_register_dump_put),
#endif
...
static int cs35l45_dai_set_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	dev_dbg(cs35l45->dev, "%s: fmt = 0x%x\n", __func__, fmt);
				    ...
				    int slots, int slot_width)
{
	...
	dev_dbg(cs35l45->dev, "%s: slot_width = %d\n", __func__, slot_width);
...
static int cs35l45_component_probe(struct snd_soc_component *component)
{
	...
	/* For kernek up linux kernel version 5,10,70 */
	...
	if (0) {
		...
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s", component->name_prefix, "DSP1 Power");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		...
		snd_soc_dapm_ignore_suspend(dapm, widget);
		snprintf(widget, sizeof(widget), "%s %s", component->name_prefix, "DSP1 Enable");
		snd_soc_dapm_ignore_suspend(dapm, widget);
		...
		snd_soc_dapm_ignore_suspend(dapm, "DSP1 Power");
		...
		snd_soc_dapm_ignore_suspend(dapm, "DSP1 Enable");
...
static irqreturn_t cs35l45_amp_short_error(int irq, void *data)
{
	struct cs35l45_private *cs35l45 = data;

	dev_dbg(cs35l45->dev, "Amplifier short error detected!");

	return IRQ_HANDLED;
}

static irqreturn_t cs35l45_bst_short_error(int irq, void *data)
{
	struct cs35l45_private *cs35l45 = data;

	dev_dbg(cs35l45->dev, "Boost inductor short error detected!");

	return IRQ_HANDLED;
}

static irqreturn_t cs35l45_bst_uvp_error(int irq, void *data)
{
	struct cs35l45_private *cs35l45 = data;

	dev_dbg(cs35l45->dev, "Boost under voltage error detected!");

	return IRQ_HANDLED;
}

static irqreturn_t cs35l45_temp_error(int irq, void *data)
{
	struct cs35l45_private *cs35l45 = data;

	dev_dbg(cs35l45->dev, "Over temperature error detected!");

	return IRQ_HANDLED;
}

static irqreturn_t cs35l45_uvlo_vddbatt_error(int irq, void *data)
{
	struct cs35l45_private *cs35l45 = data;

	dev_dbg(cs35l45->dev, "UVLO VDD_BATT voltage error detected!");

	return IRQ_HANDLED;
}

static irqreturn_t cs35l45_uvlo_vddlv_error(int irq, void *data)
{
	struct cs35l45_private *cs35l45 = data;

	dev_dbg(cs35l45->dev, "UVLO VDD_LV voltage error detected!");
...
static const struct cs35l45_irq cs35l45_irqs[] = {
	...
	CS35L45_IRQ(AMP_SHORT_ERROR, "AMP short error", cs35l45_amp_short_error),
	CS35L45_IRQ(BST_SHORT_ERROR, "BST short error", cs35l45_bst_short_error),
	CS35L45_IRQ(BST_UVP_ERROR, "BST UVP error", cs35l45_bst_uvp_error),
	CS35L45_IRQ(TEMP_ERROR, "TEMP error", cs35l45_temp_error),
	CS35L45_IRQ(UVLO_VDDBATT_ERROR, "UVLO VDDBATT error", cs35l45_uvlo_vddbatt_error),
	CS35L45_IRQ(UVLO_VDDLV_ERROR, "UVLO VDDLV error", cs35l45_uvlo_vddlv_error),
};
...
static const struct regmap_irq cs35l45_reg_irqs[] = {
	...
	CS35L45_REG_IRQ(IRQ1_EINT_1, AMP_SHORT_ERROR),
	CS35L45_REG_IRQ(IRQ1_EINT_1, BST_SHORT_ERROR),
	CS35L45_REG_IRQ(IRQ1_EINT_1, BST_UVP_ERROR),
	CS35L45_REG_IRQ(IRQ1_EINT_1, TEMP_ERROR),
	CS35L45_REG_IRQ(IRQ1_EINT_1, UVLO_VDDBATT_ERROR),
	CS35L45_REG_IRQ(IRQ1_EINT_18, UVLO_VDDLV_ERROR),
};
...
int cs35l45_initialize(struct cs35l45_private *cs35l45)
{
	...
	dev_info(dev, "Cirrus Logic CS35L45 (%x), Revision: %02X\n", dev_id,
		 rev_id);
	printk("Cirrus Logic CS35L45 (%x), Revision: %02X\n", dev_id,
		 rev_id);
