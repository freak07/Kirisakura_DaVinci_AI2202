...
extern  struct extcon_asus_dev	*quickchg_extcon;

#ifdef CONFIG_USB_HUB_USB3803_MODULE
extern void usb_path_switch(int val);
#endif
...
extern void rt1715_role_switch(enum usb_role role);
...
static int rt_chg_handle_source_vbus(struct tcp_notify *tcp_noti)
{
	...
	rc = BTM_OTG_EN(enable && !usb2_stopPower && !usb2_stopPower_screen);
	if (rc) {
		pr_info("%s failed to control GPIO BTM_OTG_EN & HAPTICS_BOOST_VREG_EN\n", __func__);
		return -1;
	}
...
int rt1715_glink_write(struct rt_charger_info *info, void *data, int len)
{
	...
	if (!rc) {
		...
		if (!rc) {
			pr_err("%s time out sending message\n", __func__);
...
void rt1715_pdo_notify(void) {
	...
	if (!tcpc) {
		pr_info("%s: get rt1711-tcpc fail\n", __func__);
	}
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_PE_READY_SNK_PD30:
			...
			if (info->peer_usb_comm && tcpc_pre_typec_state == TYPEC_UNATTACHED)
				rt1715_role_switch(USB_ROLE_DEVICE);
			break;
	...
	case TCP_NOTIFY_DR_SWAP:
		...
		if (tcpc_swap_state==1){
		    ...
		    if (info->peer_usb_comm) 
				rt1715_role_switch(USB_ROLE_HOST);
		}
		break;
...
#ifdef CONFIG_USB_HUB_USB3803_MODULE
			if (gamepad_active)
				usb_path_switch(1); //SIDE
#endif
			rt1715_role_switch(USB_ROLE_NONE);
		}
...
static int rt_host_notifier(struct notifier_block *nb,
	...
	if (usb1_active && usb2_active && (event == USB_DEVICE_ADD)){
