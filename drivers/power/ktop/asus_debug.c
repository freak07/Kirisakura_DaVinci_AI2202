...
static cputime64_t asus_get_idle_time(int cpu)
{
        cputime64_t idle;
...
static u64 asus_get_idle_time(int cpu)
{
...
static void get_all_cpustat(struct kernel_cpustat *cpu_stat)
{
	...
	for_each_possible_cpu(cpu) {
		...
		cpu_stat->cpustat[CPUTIME_IDLE] += asus_get_idle_time(cpu);
...
static void asus_kernel_top_show(struct _asus_kernel_top *ktop, int type)
{
	...
	for (i = 0; i < NUM_BUSY_THREAD_CHECK; i++) {
		if (ktop->cpustat_time > 0) {
			...
			if(g_Asus_Top1_read == 0 && (ktop->curr_proc_delta[top_n_pid] * 100 / ktop->cpustat_time) < 200 && (ktop->curr_proc_delta[top_n_pid] * 100 / ktop->cpustat_time) > 1){
				snprintf(top1_buff, 256, "[K]%s%6lu.%d%%\t%d\t%s\t\t%llu\n", type == KERNEL_TOP_ACCU ? "[KTOP]" : " ",
				ktop->curr_proc_delta[top_n_pid] * 100 / ktop->cpustat_time,
				(ktop->curr_proc_delta[top_n_pid] * 1000 / ktop->cpustat_time) % 10,
				top_n_pid,
				ktop->task_ptr_array[top_n_pid]->comm,
...
static const struct proc_ops proc_dumpTop1ToDropbox_operations = {
        
		.proc_read   = dumpTop1ToDropbox_read,
};
...
static void __exit asus_cpu_monitor_exit(void)
{
	printk("KTOP exit.\n");
}

module_init(asus_cpu_monitor_init);
module_exit(asus_cpu_monitor_exit);

MODULE_DESCRIPTION("ASUS KTOP debug module");
