...
//#define BC_WAIT_TIME_MS			1000 //ASUS_BSP move to asus_battery_charger.c
...
//ASUS_BSP +++
//extern int asus_chg_resume(struct device *dev);
...
extern int qti_cell1_voltage;  //shawn
extern int qti_cell2_voltage;
...
//ASUS_BSP : remove the static use of battery_chg_write, move to asus_battery_charger.c
...
//ASUS_BSP remove
	...
	if (pst->prop[USB_ADAP_TYPE] != POWER_SUPPLY_USB_TYPE_SDP &&
		...
		bcdev->usb_icl_ua = 0;
#endif
	pr_err("usb_adap_type: %u, usb_online=%d\n", pst->prop[USB_ADAP_TYPE], pst->prop[USB_ONLINE]);
				...
				size_t len)
{
	...
	if (pst && pst->psy) {
		...
		if (g_Charger_mode && notify_msg->notification == BC_USB_STATUS_GET) {
			pm_wakeup_dev_event(bcdev->dev, 10000, true);
		} else {
		...
		union power_supply_propval *pval)
{
	...
	if (prop == POWER_SUPPLY_PROP_ONLINE && pval->intval == 0) {	// ASUS BSP: set online = 1 when feature usbin_suspend with cable
		if (g_vbus_plug){
			pr_err("[BAT][CHG] USB_ONLINE=0, but vbus=%d\n", g_vbus_plug);
			pval->intval = 1;
		} else {
...
//ASUS_BSP ---
	...
	case POWER_SUPPLY_PROP_MODEL_NAME:
		...
		strcpy(model_name, pst->model); //ASUS_BSP +
		break;
	...
	case POWER_SUPPLY_PROP_TEMP:
		pval->intval = pst->prop[prop_id] - 2731;
		break;
...
//ASUS_BSP +++
	...
	case POWER_SUPPLY_PROP_HEALTH:  
		...
		if (qti_cell1_voltage < 2800 || qti_cell2_voltage < 2800)  //2800
		{							//health_type[bat_health])  
			pval->intval = 4;   	//"Over voltage" = 4  => bat_health=4;
			pr_err("[BAT][CHG] shawn qit_battery_charger, Vcell1:%dmV, Vcell2:%dmV\n", qti_cell1_voltage,qti_cell2_voltage);
		}
		else
			pval->intval = pst->prop[prop_id];	
		break;
...
//	POWER_SUPPLY_PROP_TIME_TO_FULL_NOW, //ASUS_BSP -
...
//	req_msg.fw_crc = bcdev->wls_fw_crc; //ASUS_BSP -
...
//	&class_attr_wireless_fw_crc.attr, //ASUS_BSP -
...
//	bcdev->initialized = true; //ASUS_BSP -
...
//	schedule_work(&bcdev->usb_type_work); //ASUS_BSP -
	...
	return 0;
error:
//	bcdev->initialized = false; //ASUS_BSP -
//	complete(&bcdev->ack); //ASUS_BSP -
