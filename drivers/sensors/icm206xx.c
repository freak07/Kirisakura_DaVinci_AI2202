...
static int icm_config_sample_rate(struct icm_sensor *sensor, int sensor_type, bool set_pwr);
...
static int icm_gyro_do_enable(struct icm_sensor *sensor, bool enable)
{
	...
	mutex_lock(&sensor->op_lock);
	if (enable) {
		ret = icm_config_sample_rate(sensor, SNS_TYPE_GYRO, false);
		if (ret < 0) {
...
static int icm_config_sample_rate(struct icm_sensor *sensor, int sensor_type, bool set_pwr)
{
	...
	if (sensor->cfg.is_asleep)
		return -EINVAL;
	if (!atomic_read(&sensor->gyro_en) && sensor_type != SNS_TYPE_GYRO) {
		delay_ms = sensor->accel_poll_ms;
	} else if (!atomic_read(&sensor->accel_en) && sensor_type != SNS_TYPE_ACCEL) {
					...
					unsigned long delay)
{
	...
	if (sensor->use_poll) {
		...
		ret = icm_config_sample_rate(sensor, SNS_TYPE_GYRO, true);
		if (ret < 0)
...
static int icm_accel_do_enable(struct icm_sensor *sensor, bool enable)
{
	...
	mutex_lock(&sensor->op_lock);
	if (enable) {
		ret = icm_config_sample_rate(sensor, SNS_TYPE_ACCEL, false);
		if (ret < 0) {
			icm_errmsg("Unable to update sampling rate! ret=%d\n", ret);
		}
					...
					unsigned long delay)
{
	...
	if (sensor->use_poll) {
		...
		ret = icm_config_sample_rate(sensor, SNS_TYPE_ACCEL, true);
		if (ret < 0)
