...
void asus_init_state(void){
	...
	if(snt8100fsr_g->grip_fw_loading_status != false){
		...
		write_register(snt8100fsr_g, REGISTER_TIRG_CTRL1, &trig3_control);
		...
		write_register(snt8100fsr_g, REGISTER_TIRG_CTRL2, &trig5_control);
		...
		*/
		...
		for(j=0;j<squeeze_algo_num;j++){
			...
			set_sq_gesture(j, SQ_BIT9[j], 9);
			set_sq_gesture(j, SQ_BIT10[j], 10);
		}
...
int start_event_processing(struct snt8100fsr *snt8100fsr) {
	...
	if(snt8100fsr_g->grip_fw_loading_status){
		asus_init_state();
...
*/
	...
	* are not equal to zero, maybe there is better way to stop the while-loop */
	while ((reg_id != 0x00) && (len != 0x00)) {
		PRINT_DEBUG("Enter while: reg_id = %d, len = %d, times = %d", reg_id, len, times);
	...
	PRINT_INFO("write 0x1=1 to prevent cal data bug");
	reg_enable.enable = 1;
	ret = write_register(snt8100fsr, REGISTER_ENABLE, &reg_enable);
	if (ret) {
		PRINT_CRIT("sb_read_register() failed");
