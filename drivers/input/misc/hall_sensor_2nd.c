...
#define hall2_info(fmt, arg...)	pr_info("[hall_2nd][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define	hall2_err(fmt, arg...)	pr_err("[hall_2nd][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)

/*****************************/
/* Hall Sensor Configuration */
/****************************/
#define DRIVER_NAME 		"hall2_sensor"
#define IRQ_Name			"ASUS_Hall2_Sensor-irq"
#define INT_NAME			"Hall2Sensor_INT"
...
struct delayed_work RegDRMQueen;
static unsigned char ReportFlg = 1;	   //1: report, 0: not report.  default report.

static void RegisterDRMCallback(void);
         ...
         struct panel_event_notification *notification, void *client_data)
{
    ...
    case DRM_PANEL_EVENT_UNBLANK:
		 ReportFlg = 1;
		 hall2_info("Display on.\n");
        break;
		
    case DRM_PANEL_EVENT_BLANK:
		 ReportFlg = 0;
		 hall2_info("Display off.\n");
        break;
...
void RegDRM_worker(struct work_struct *work)
{
	hall2_info("%s.\n", __func__);
	RegisterDRMCallback();
}
...
static int drm_check_dt(struct device_node *np)
{
    ...
    if (count <= 0) {
		hall2_err("%s find drm_panel count(%d) fail!\n", __func__, count);
        return -ENODEV;
    }

	for (retry = 20;retry >0;retry--){
		hall2_err("%s retry count(%d)\n", __func__, retry);
		for (i = 0; i < count; i++) {
			node = of_parse_phandle(np, "panel", i);
			panel = of_drm_find_panel(node);
			of_node_put(node);
			if (!IS_ERR(panel)) {
				hall2_err("%s find drm_panel successfully!\n", __func__);
				active_panel = panel;
				return 0;
			}
		}
		msleep(200);		//Need modify...
	}

	hall2_err("%s no find drm_panel!\n", __func__);
...
{	
	...
	hall2_info("%s.\n", __func__);
    ret = drm_check_dt(g_pdev->dev.of_node);
    if (ret) {
		hall2_err("%s parse drm-panel fail!\n", __func__);
    }

    if (active_panel) {
		hall2_err("%s registering fb notification!\n", __func__);
        cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
                PANEL_EVENT_NOTIFIER_CLIENT_HALL, active_panel,
                &fts_ts_panel_notifier_callback, &ts_data);
        if (!cookie) {
			hall2_err("%s Failed to register for panel events!\n", __func__);
            return;
        }
    }
...
//	__pm_wakeup_event(hall_sensor_dev->wake_src, REPORT_WAKE_LOCK_TIMEOUT);
	...
	if(!hall_sensor_dev->enable){
		hall2_info("%s hall sensor is disable!\n", __func__);
...
//		msleep(50);
//		hall2_info("%s hall sensor counter:%d \n", __func__, counter);
//		if (gpio_get_value(ASUS_HALL_SENSOR_GPIO) == 1) {
//			hall_sensor_dev->status = 1;
//			counter_trigger++;
//			hall2_info("%s gpio_get_value 1 \n", __func__);
//		} else {
//			hall_sensor_dev->status = 0;
//			hall2_info("%s gpio_get_value 0 \n", __func__);
//		}
//	}
//	if( (counter_trigger > 0) && (counter_trigger < 2)){
//		hall2_info("%s SW_LID do not report to framework.\n", __func__);
//		hall_sensor_dev->status = initial_status;
//		__pm_relax(hall_sensor_dev->wake_src);
//		return;
//	}
	hall_sensor_dev->status = gpio_get_value(ASUS_HALL_SENSOR_GPIO);
	hall2_info("%s Hall sensor status = %d", __func__, hall_sensor_dev->status);
	mutex_unlock(&hall_sensor_dev->hall_mutex);
	
	if(1){		//ReportFlg == 1
		input_report_switch(hall_sensor_dev->hall_indev, SW_PEN_INSERTED, !hall_sensor_dev->status);
		...
		hall2_info("%s ingore report current status(%d)", __func__, hall_sensor_dev->status);
	}
	
	__pm_relax(hall_sensor_dev->wake_src);
}

int report_hall2_status(void)
{
	if(!hall_sensor_dev) {
		hall2_err("%s Hall sensor does not exist!\n", __func__);
		return -1; //hall sensor is not
	}
	return hall_sensor_dev->status;
}
EXPORT_SYMBOL(report_hall2_status);

static ssize_t show_action_status(struct device *dev,struct device_attribute *attr, char *buf)
{
	if(!hall_sensor_dev)
		return sprintf(buf, "Hall2 sensor does not exist!\n");
...
static ssize_t store_action_status(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	hall2_info("%s status rewite value = %d\n", __func__, !hall_sensor_dev->status);
	return count;
}
...
static ssize_t store_hall_sensor_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(request==hall_sensor_dev->enable){
		...
		if(0 == request) {
			/* Turn off */
			hall2_info("%s Turn off.\n", __func__);
			mutex_lock(&hall_sensor_dev->hall_mutex);
			hall_sensor_dev->enable=request;
			mutex_unlock(&hall_sensor_dev->hall_mutex);
		}else if(1 == request){
			/* Turn on */
			hall2_info("%s Turn on. \n", __func__);
			mutex_lock(&hall_sensor_dev->hall_mutex);
			hall_sensor_dev->enable=request;
			mutex_unlock(&hall_sensor_dev->hall_mutex);
		}else{
			hall2_err("%s Enable/Disable Error, can not recognize (%d)", __func__, request);
		}
	}
	return count;
}
...
static int init_input_event(void)
{
	...
	if(!hall_sensor_dev->hall_indev){
		hall2_err("%s Failed to allocate input event device\n", __func__);
		return -ENOMEM;
	}

	hall_sensor_dev->hall_indev->name = "hall_input2";
	hall_sensor_dev->hall_indev->phys= "/dev/input/hall_indev";
	hall_sensor_dev->hall_indev->dev.parent= NULL;
	input_set_capability(hall_sensor_dev->hall_indev, EV_SW, SW_PEN_INSERTED);

	ret = input_register_device(hall_sensor_dev->hall_indev);
	if (ret) {
		hall2_err("%s Failed to register input event device\n", __func__);
		return -1;
	}

	hall2_err("%s Input Event registration Success!\n", __func__);
	return 0;
}
...
static void set_pinctrl(struct device *dev)
{
	...
	hall2_info("%s: pinctrl_select_state = %d\n", __func__, ret);
}
...
static int init_irq (void)
{
	...
	if (ASUS_HALL_SENSOR_IRQ < 0) {
		hall2_err("%s gpio_to_irq ERROR, irq=%d.\n", __func__, ASUS_HALL_SENSOR_IRQ);
	}else {
		hall2_err("%s gpio_to_irq IRQ %d successed on GPIO:%d\n", __func__, ASUS_HALL_SENSOR_IRQ, ASUS_HALL_SENSOR_GPIO);
	}
	...
	if (ret < 0)
		hall2_err("%s request_irq() ERROR %d.\n", __func__, ret);
	else {
		hall2_err("%s Enable irq !! \n", __func__);
...
static int hall_sensor_probe(struct platform_device *pdev)
{
	...
	hall2_err("%s Hall sensor probe  +++\n", __func__);

	/* Initialization Data */
	hall_sensor_dev = kzalloc(sizeof (struct hall_sensor_str), GFP_KERNEL);
	if (!hall_sensor_dev) {
		hall2_err("%s Memory allocation fails for hall sensor\n", __func__);
		return -ENOMEM;
	}
	mutex_init(&hall_sensor_dev->hall_mutex);

	/* GPIO */
	ASUS_HALL_SENSOR_GPIO = of_get_named_gpio(pdev->dev.of_node, "qcom,hall2-gpio", 0);
	hall2_err("%s Hall sensor [GPIO] GPIO =%d(%d)\n", __func__, ASUS_HALL_SENSOR_GPIO, gpio_get_value(ASUS_HALL_SENSOR_GPIO));
	...
	if (ret) {
		hall2_err("%s Hall sensor [GPIO] Unable to request gpio %s(%d)\n", __func__, IRQ_Name, ASUS_HALL_SENSOR_GPIO);
		goto probe_err;
	}
	...
	if (ret < 0) {
		hall2_err("%s Hall sensor [GPIO] Unable to set the direction of gpio %d\n", __func__, ASUS_HALL_SENSOR_GPIO);
		goto probe_err;
	}
	...
	if (IS_ERR(hall_sensor_dev->vdd_supply)) {
		hall2_err("%s Hall sensor ret vdd regulator failed,ret=%d", __func__, ret);
		goto probe_err;
	}
	ret = regulator_enable(hall_sensor_dev->vdd_supply);
	if (ret) {
		hall2_err("%s Hall sensor enable vcc_i2c regulator failed,ret=%d", __func__, ret);
	}
	msleep(10);
	#endif

	//remove input event since no need
	...
	if (ret) {
		hall2_err("%s Hall sensor sysfs_create_group ERROR.\n", __func__);
		goto probe_err;
	}
	...
	INIT_DELAYED_WORK(&RegDRMQueen, RegDRM_worker);
	schedule_delayed_work(&RegDRMQueen, 0);	
	hall2_err("%s Hall sensor probe  ---\n", __func__);
	return 0;

probe_err:
	hall2_err("%s Hall sensor probe error\n", __func__);
	return ret;
...
static int __init hall_sensor_init(void)
{
	...
	hall2_info("%s Hall sensor driver init +++\n", __func__);
	err = platform_driver_register(&hall_sensor_driver);
	if (err != 0) {
		hall2_err("%s Hall sensor platform_driver_register fail, error=%d\n", __func__, err);
		return err;
    }
	hall2_info("%s Hall sensor driver init ---\n", __func__);
	return err;
...
static void __exit hall_sensor_exit(void)
{
	...
	hall2_info("%s Hall sensor driver exit +++\n", __func__);
	ret = regulator_disable(hall_sensor_dev->vdd_supply);
	if (ret)
		hall2_err("%s disable ibb regulator failed,ret=%d\n", __func__, ret);
...
MODULE_DESCRIPTION("Hall2 Sensor");
