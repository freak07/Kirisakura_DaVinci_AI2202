...
#define hall1_info(fmt, arg...)	pr_info("[hall_1nd][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define	hall1_err(fmt, arg...)	pr_err("[hall_1nd][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)

/*****************************/
/* Hall Sensor Configuration */
/****************************/
#define DRIVER_NAME 		"hall1_sensor"
#define IRQ_Name			"ASUS_Hall1_Sensor-irq"
#define INT_NAME			"Hall1Sensor_INT"
...
//	__pm_wakeup_event(hall_sensor_dev->wake_src, REPORT_WAKE_LOCK_TIMEOUT);
	...
	if(!hall_sensor_dev->enable){
		hall1_info("%s hall sensor is disable!\n", __func__);
...
//		msleep(50);
//		hall1_info("%s hall sensor counter:%d \n", __func__, counter);
//		if (gpio_get_value(ASUS_HALL_SENSOR_GPIO) == 1) {
//			hall_sensor_dev->status = 1;
//			counter_trigger++;
//			hall1_info("%s gpio_get_value 1 \n", __func__);
//		} else {
//			hall_sensor_dev->status = 0;
//			hall1_info("%s gpio_get_value 0 \n", __func__);
//		}
//	}
//	if( (counter_trigger > 0) && (counter_trigger < 2)){
//		hall1_info("%s SW_LID do not report to framework.\n", __func__);
//		hall_sensor_dev->status = initial_status;
//		__pm_relax(hall_sensor_dev->wake_src);
//		return;
//	}
	hall_sensor_dev->status = gpio_get_value(ASUS_HALL_SENSOR_GPIO);
	hall1_info("%s Hall sensor status = %d", __func__, hall_sensor_dev->status);
	mutex_unlock(&hall_sensor_dev->hall_mutex);
	
	input_report_switch(hall_sensor_dev->hall_indev, SW_DOCK, !hall_sensor_dev->status);
...
int report_hall1_status(void)
{
	if(!hall_sensor_dev) {
		hall1_err("%s Hall sensor does not exist!\n", __func__);
		return -1; //hall sensor is not
	}
	return hall_sensor_dev->status;
}
EXPORT_SYMBOL(report_hall1_status);

static ssize_t show_action_status(struct device *dev,struct device_attribute *attr, char *buf)
{
	if(!hall_sensor_dev)
		return sprintf(buf, "Hall1 sensor does not exist!\n");
...
static ssize_t store_action_status(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	hall1_info("%s status rewite value = %d\n", __func__, !hall_sensor_dev->status);
	return count;
}
...
static ssize_t store_hall_sensor_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(request==hall_sensor_dev->enable){
		...
		if(0 == request) {
			/* Turn off */
			hall1_info("%s Turn off.\n", __func__);
			mutex_lock(&hall_sensor_dev->hall_mutex);
			hall_sensor_dev->enable=request;
			mutex_unlock(&hall_sensor_dev->hall_mutex);
		}else if(1 == request){
			/* Turn on */
			hall1_info("%s Turn on. \n", __func__);
			mutex_lock(&hall_sensor_dev->hall_mutex);
			hall_sensor_dev->enable=request;
			mutex_unlock(&hall_sensor_dev->hall_mutex);
		}else{
			hall1_err("%s Enable/Disable Error, can not recognize (%d)", __func__, request);
		}
	}
	return count;
}
...
static int init_input_event(void)
{
	...
	if(!hall_sensor_dev->hall_indev){
		hall1_err("%s Failed to allocate input event device\n", __func__);
		return -ENOMEM;
	}

	hall_sensor_dev->hall_indev->name = "hall_input1";
	hall_sensor_dev->hall_indev->phys= "/dev/input/hall_indev";
	hall_sensor_dev->hall_indev->dev.parent= NULL;
	input_set_capability(hall_sensor_dev->hall_indev, EV_SW, SW_DOCK);

	ret = input_register_device(hall_sensor_dev->hall_indev);
	if (ret) {
		hall1_err("%s Failed to register input event device\n", __func__);
		return -1;
	}

	hall1_err("%s Input Event registration Success!\n", __func__);
	return 0;
}
...
static void set_pinctrl(struct device *dev)
{
	...
	set_state = pinctrl_lookup_state(key_pinctrl, "hall1_gpio_high");
	ret = pinctrl_select_state(key_pinctrl, set_state);
	hall1_info("%s: pinctrl_select_state = %d\n", __func__, ret);
}
...
static int init_irq (void)
{
	...
	if (ASUS_HALL_SENSOR_IRQ < 0) {
		hall1_err("%s gpio_to_irq ERROR, irq=%d.\n", __func__, ASUS_HALL_SENSOR_IRQ);
	}else {
		hall1_err("%s gpio_to_irq IRQ %d successed on GPIO:%d\n", __func__, ASUS_HALL_SENSOR_IRQ, ASUS_HALL_SENSOR_GPIO);
	}
	...
	if (ret < 0)
		hall1_err("%s request_irq() ERROR %d.\n", __func__, ret);
	else {
		hall1_err("%s Enable irq !! \n", __func__);
...
static int hall_sensor_probe(struct platform_device *pdev)
{
	...
	hall1_err("%s Hall sensor probe  +++\n", __func__);

	/* Initialization Data */
	hall_sensor_dev = kzalloc(sizeof (struct hall_sensor_str), GFP_KERNEL);
	if (!hall_sensor_dev) {
		hall1_err("%s Memory allocation fails for hall sensor\n", __func__);
		return -ENOMEM;
	}
	mutex_init(&hall_sensor_dev->hall_mutex);

	/* GPIO */
	ASUS_HALL_SENSOR_GPIO = of_get_named_gpio(pdev->dev.of_node, "qcom,hall1-gpio", 0);
	hall1_err("%s Hall sensor [GPIO] GPIO =%d(%d)\n", __func__, ASUS_HALL_SENSOR_GPIO, gpio_get_value(ASUS_HALL_SENSOR_GPIO));
	...
	if (ret) {
		hall1_err("%s Hall sensor [GPIO] Unable to request gpio %s(%d)\n", __func__, IRQ_Name, ASUS_HALL_SENSOR_GPIO);
		goto probe_err;
	}
	...
	if (ret < 0) {
		hall1_err("%s Hall sensor [GPIO] Unable to set the direction of gpio %d\n", __func__, ASUS_HALL_SENSOR_GPIO);
		goto probe_err;
	}
	...
	if (IS_ERR(hall_sensor_dev->vdd_supply)) {
		hall1_err("%s Hall sensor ret vdd regulator failed,ret=%d", __func__, ret);
		goto probe_err;
	}
	ret = regulator_enable(hall_sensor_dev->vdd_supply);
	if (ret) {
		hall1_err("%s Hall sensor enable vcc_i2c regulator failed,ret=%d", __func__, ret);
	}
	msleep(10);
	#endif
	...
	if (ret) {
		hall1_err("%s Hall sensor sysfs_create_group ERROR.\n", __func__);
		goto probe_err;
	}
...
probe_err:
	hall1_err("%s Hall sensor probe error\n", __func__);
	return ret;
...
static int __init hall_sensor_init(void)
{
	...
	hall1_info("%s Hall sensor driver init +++\n", __func__);
	err = platform_driver_register(&hall_sensor_driver);
	if (err != 0) {
		hall1_err("%s Hall sensor platform_driver_register fail, error=%d\n", __func__, err);
		return err;
    }
	hall1_info("%s Hall sensor driver init ---\n", __func__);
	return err;
...
static void __exit hall_sensor_exit(void)
{
	...
	hall1_info("%s Hall sensor driver exit +++\n", __func__);
	ret = regulator_disable(hall_sensor_dev->vdd_supply);
	if (ret)
		hall1_err("%s disable ibb regulator failed,ret=%d\n", __func__, ret);
...
MODULE_DESCRIPTION("Hall1 Sensor");
