...
ASUS BSP Pin Ctrl --- */
	...
	if (rc)
		printk("[FP][%s] reset gpio pull down failed, rc = %d\n", __func__, rc);
	else
		printk("[FP][%s] reset gpio pull down!\n", __func__);
	...
	if (rc)
		printk("[FP][%s] irq gpio set to input failed, rc = %d\n", __func__, rc);
	else
		printk("[FP][%s] irq gpio set to input !\n", __func__);
		
	r = gpio_request(340, "goodix_cs");
	if (r) {
		pr_err("[FP][%s]failed to request irq gpio, rc = %d\n", __func__, r);
	}
	
	r = gpio_direction_output(340, 0);
	if (r) {
		printk("[FP][%s] cs gpio pull down failed, r = %d\n", __func__, r);
	} else {
		printk("[FP][%s] cs gpio pull down!\n", __func__);
	}
...
int gf_power_on(struct gf_dev *gf_dev)
{
	...
	if (gpio_is_valid(gf_dev->reset_gpio)) {
		...
		if (rc)
			printk("[FP][%s] reset gpio pull up failed, rc = %d\n", __func__, rc);
		else
			printk("[FP][%s] reset gpio pull up!\n", __func__);
	}
