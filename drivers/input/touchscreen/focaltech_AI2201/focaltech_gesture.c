...
//    struct input_dev *input_dev = ts_data->input_dev;
    ...
    FTS_INFO("fts read result %d",ret);
    
    if ((ret < 0) || (buf[0] != ENABLE)) {
        FTS_DEBUG("gesture not enable in fw, don't process gesture %d",buf[0]);
        return 1;
    }
    ...
    asus_gesture_report(ts_data, gesture->gesture_id);
    return 0;
}
