...
void fts_release_all_finger(void)
{
	...
	clear_atr();
	for (finger_count = 0; finger_count < max_touches + TOTAL_GAME_USED_SLOT; finger_count++) {
 	    input_mt_slot(input_dev, finger_count+10);
...
#if FTS_MT_PROTOCOL_B_EN
    input_mt_init_slots(input_dev, pdata->max_touch_number+TOTAL_GAME_USED_SLOT, INPUT_MT_DIRECT);
#else
