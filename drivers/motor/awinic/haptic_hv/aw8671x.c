...
{
	if (flag) {
		i2c_w_bits(aw_haptic, AW8671X_REG_I2SCFG1,
			   ...
			   struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		i2c_w_bits(aw_haptic, AW8671X_REG_SYSCTRL2,
			     ...
			     struct snd_soc_dai *dai)
{
	...
	/* set width */
	i2c_w_bits(aw_haptic, AW8671X_REG_I2SCFG3,
	...
	/* check i2s cfg */
	i2c_r_bytes(aw_haptic, AW8671X_REG_I2SCFG3, &tmp_val, AW_I2C_BYTE_ONE);
...
static int aw8671x_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		i2c_r_bytes(aw_haptic, AW8671X_REG_SYSST, &reg_val,
			    AW_I2C_BYTE_ONE);
		aw_info("sysst=0x%02X", reg_val);
		i2c_r_bytes(aw_haptic, AW8671X_REG_SYSST2, &reg_val,
			    AW_I2C_BYTE_ONE);
		aw_info("sysst2=0x%02X", reg_val);
		i2c_r_bytes(aw_haptic, AW8671X_REG_SYSER, &reg_val,
			    AW_I2C_BYTE_ONE);
...
static int aw8671x_check_qualify(struct aw_haptic *aw_haptic)
{
	...
	aw_info("enter");
	ret = i2c_r_bytes(aw_haptic, AW8671X_REG_EFCFG5, &reg_val,
			  AW_I2C_BYTE_ONE);
	if (ret < 0)
		return ret;
	if (!(reg_val & 0x80)) {
...
{
	i2c_w_bytes(aw_haptic, AW8671X_REG_PLAYCFG2, &gain, AW_I2C_BYTE_ONE);
}
...
static void aw8671x_set_bst_vol(struct aw_haptic *aw_haptic, uint32_t bst_vol)
{
	...
	if (bst_vol < AW8671X_CHARGEPUMP_CODE1)
		...
		reg_val = 0x04;
	i2c_w_bits(aw_haptic, AW8671X_REG_PLAYCFG1,
...
static void aw8671x_set_wav_seq(struct aw_haptic *aw_haptic, uint8_t wav,
				uint8_t seq)
{
	i2c_w_bytes(aw_haptic, AW8671X_REG_WAVCFG1 + wav, &seq,
		    AW_I2C_BYTE_ONE);
}
				 ...
				 uint8_t loop)
{
	...
	if (wav % 2) {
		tmp = loop << 0;
		i2c_w_bits(aw_haptic, AW8671X_REG_WAVCFG9 + (wav / 2),
...
static void aw8671x_set_rtp_data(struct aw_haptic *aw_haptic, uint8_t *data,
				 uint32_t len)
{
	i2c_w_bytes(aw_haptic, AW8671X_REG_RTPDATA, data, len);
}

static void aw8671x_set_rtp_aei(struct aw_haptic *aw_haptic, bool flag)
{
	if (flag) {
		i2c_w_bits(aw_haptic, AW8671X_REG_SYSINTM,
...
static void aw8671x_set_ram_addr(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_RAMADDRH,
		   AW8671X_BIT_RAMADDRH_MASK,
		   (uint8_t)(aw_haptic->ram.base_addr >> 8));
	i2c_w_bytes(aw_haptic, AW8671X_REG_RAMADDRL, &ram_addr_l,
		    AW_I2C_BYTE_ONE);
}
...
{
	if (flag) {
		i2c_w_bits(aw_haptic, AW8671X_REG_PLAYCFG3,
...
{
	if (flag) {
		i2c_w_bits(aw_haptic, AW8671X_REG_CONTCFG1,
...
static uint8_t aw8671x_get_glb_state(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_GLBRD5, &state, AW_I2C_BYTE_ONE);
...
static void aw8671x_play_go(struct aw_haptic *aw_haptic, bool flag)
{
	...
	if (flag) {
		if (aw_haptic->info.is_enabled_one_wire) {
			i2c_w_bytes(aw_haptic, AW8671X_REG_GLBCFG2,
				    &delay_20us, AW_I2C_BYTE_ONE);
			i2c_w_bytes(aw_haptic, AW8671X_REG_PLAYCFG4,
...
{
	if (flag) {
		i2c_w_bits(aw_haptic, AW8671X_REG_SYSCTRL1,
...
static void aw8671x_upload_lra(struct aw_haptic *aw_haptic, uint32_t flag)
{
	...
	case AW_WRITE_ZERO:
		...
		i2c_w_bits(aw_haptic, AW8671X_REG_TRIMCFG2,
...
static uint8_t aw8671x_get_trim_lra(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_TRIMCFG2, &reg_val, AW_I2C_BYTE_ONE);
	reg_val &= 0x3F;
	return reg_val;
}
...
static void aw8671x_vbat_mode_config(struct aw_haptic *aw_haptic, uint8_t flag)
{
	if (flag == AW_CONT_VBAT_HW_COMP_MODE) {
		i2c_w_bits(aw_haptic, AW8671X_REG_VBATCTRL,
...
static void aw8671x_protect_config(struct aw_haptic *aw_haptic,
				   uint8_t addr, uint8_t val)
{
	aw_info("enter");
	if (addr == 1) {
		i2c_w_bits(aw_haptic, AW8671X_REG_PWMCFG2,
			   AW8671X_BIT_PWMCFG2_PRCT_MODE_MASK,
			   AW8671X_BIT_PWMCFG2_PRCT_MODE_VALID);
		i2c_w_bits(aw_haptic, AW8671X_REG_PWMCFG1,
			   AW8671X_BIT_PWMCFG1_PRC_EN_MASK,
			   AW8671X_BIT_PWMCFG1_PRC_ENABLE);
		i2c_w_bits(aw_haptic, AW8671X_REG_PWMCFG3,
			   AW8671X_BIT_PWMCFG3_PR_EN_MASK,
			   AW8671X_BIT_PWMCFG3_PR_ENABLE);
	} else if (addr == 0) {
		i2c_w_bits(aw_haptic, AW8671X_REG_PWMCFG2,
			   AW8671X_BIT_PWMCFG2_PRCT_MODE_MASK,
			   AW8671X_BIT_PWMCFG2_PRCT_MODE_INVALID);
		i2c_w_bits(aw_haptic, AW8671X_REG_PWMCFG1,
			   AW8671X_BIT_PWMCFG1_PRC_EN_MASK,
	...
	} else if (addr == AW8671X_REG_PWMCFG1) {
		i2c_w_bits(aw_haptic, AW8671X_REG_PWMCFG1,
			   AW8671X_BIT_PWMCFG1_PRCTIME_MASK, val);
	} else if (addr == AW8671X_REG_PWMCFG3) {
		i2c_w_bits(aw_haptic, AW8671X_REG_PWMCFG3,
			   AW8671X_BIT_PWMCFG3_PRLVL_MASK, val);
	} else if (addr == AW8671X_REG_PWMCFG4) {
		i2c_w_bytes(aw_haptic, AW8671X_REG_PWMCFG4, &val,
			    AW_I2C_BYTE_ONE);
	}
}
...
static void aw8671x_cont_config(struct aw_haptic *aw_haptic)
{
	...
	/* f0 driver level */
	i2c_w_bits(aw_haptic, AW8671X_REG_CONTCFG6,
		   (AW8671X_BIT_CONTCFG6_TRACK_EN_MASK &
		    AW8671X_BIT_CONTCFG6_DRV1_LVL_MASK),
		   (AW8671X_BIT_CONTCFG6_TRACK_ENABLE |
		    aw_haptic->info.cont_drv1_lvl));

	i2c_w_bytes(aw_haptic, AW8671X_REG_CONTCFG7,
		    &aw_haptic->info.cont_drv2_lvl, AW_I2C_BYTE_ONE);
	/* DRV1_TIME */
	/* i2c_w_bytes(aw_haptic, AW8671X_REG_CONTCFG8, &drv1_time,
		       AW_I2C_BYTE_ONE); */
	/* DRV2_TIME */
	i2c_w_bytes(aw_haptic, AW8671X_REG_CONTCFG9, &drv2_time,
		    AW_I2C_BYTE_ONE);
	/* cont play go */
...
static void aw8671x_one_wire_init(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bytes(aw_haptic, AW8671X_REG_GLBCFG4, &trig_prio,
		    AW_I2C_BYTE_ONE);
	i2c_w_bytes(aw_haptic, AW8671X_REG_GLBCFG2, &delay_2p5ms,
		    AW_I2C_BYTE_ONE);
	i2c_w_bits(aw_haptic, AW8671X_REG_TRGCFG8,
...
static void aw8671x_trig1_param_config(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_TRGCFG1,
		   (AW8671X_BIT_TRG_ENABLE_MASK & AW8671X_BIT_TRG_SEQ_MASK),
		   trig_config);
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_TRGCFG4,
		   (AW8671X_BIT_TRG_ENABLE_MASK & AW8671X_BIT_TRG_SEQ_MASK),
		   trig_config);
}
...
static void aw8671x_trig2_param_config(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_TRGCFG7,
		   (AW8671X_BIT_TRGCFG7_TRG2_MODE_MASK &
		    ...
		    AW8671X_BIT_TRGCFG7_TRG2_BST_MASK), trig_config);
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_TRGCFG2,
		   (AW8671X_BIT_TRG_ENABLE_MASK & AW8671X_BIT_TRG_SEQ_MASK),
		   trig_config);
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_TRGCFG5,
		   (AW8671X_BIT_TRG_ENABLE_MASK & AW8671X_BIT_TRG_SEQ_MASK),
		   trig_config);
}
...
static void aw8671x_trig3_param_config(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_TRGCFG8,
		   (AW8671X_BIT_TRGCFG8_TRG3_MODE_MASK &
		    ...
		    AW8671X_BIT_TRGCFG8_TRG3_BST_MASK), trig_config);
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_TRGCFG3,
		   (AW8671X_BIT_TRG_ENABLE_MASK & AW8671X_BIT_TRG_SEQ_MASK),
		   trig_config);
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_TRGCFG6,
		   (AW8671X_BIT_TRG_ENABLE_MASK & AW8671X_BIT_TRG_SEQ_MASK),
		   trig_config);
}
...
static void aw8671x_interrupt_setup(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_SYSINT, &reg_val, AW_I2C_BYTE_ONE);
	aw_info("reg SYSINT=0x%02X", reg_val);
	/* edge int mode */
	i2c_w_bits(aw_haptic, AW8671X_REG_SYSCTRL3,
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_SYSINTM,
		   ...
		   (AW8671X_BIT_SYSINTM_BST_OVPM_OFF |
		    AW8671X_BIT_SYSINTM_UVLM_ON | AW8671X_BIT_SYSINTM_OCDM_ON |
		    AW8671X_BIT_SYSINTM_OTM_ON));
}

static int aw8671x_juge_rtp_going(struct aw_haptic *aw_haptic)
{
	uint8_t glb_state = 0;
	uint8_t rtp_state = 0;
...
static void aw8671x_get_ram_data(struct aw_haptic *aw_haptic, char *buf)
{
	...
	while (i < aw_haptic->ram.len) {
		...
		i2c_r_bytes(aw_haptic, AW8671X_REG_RAMDATA, buf + i, size);
		i += size;
	}
}
					...
					uint8_t *wave_addr)
{
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_RAMDATA, reg_array,
		    AW_I2C_BYTE_THREE);
...
static void aw8671x_get_wav_seq(struct aw_haptic *aw_haptic, uint32_t len)
{
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_WAVCFG1, reg_val, len);
	for (i = 0; i < len; i++)
...
static size_t aw8671x_get_wav_loop(struct aw_haptic *aw_haptic, char *buf)
{
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_WAVCFG9, reg_val,
...
static uint8_t aw8671x_get_prctmode(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_DETCFG1, &reg_val, AW_I2C_BYTE_ONE);
	return reg_val;
...
static int aw8671x_read_f0(struct aw_haptic *aw_haptic)
{
	...
	/* lra_f0 */
	i2c_r_bytes(aw_haptic, AW8671X_REG_CONTCFG14, reg_val, AW_I2C_BYTE_TWO);
...
#else
	/* cont_f0 */
	i2c_r_bytes(aw_haptic, AW8671X_REG_CONTCFG16, reg_val, AW_I2C_BYTE_TWO);
	f0_reg = (reg_val[0] << 8) | reg_val[1];
	if (!f0_reg) {
		aw_haptic->f0 = 0;
...
static int aw8671x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	/* config max d2s_gain */
	i2c_r_bytes(aw_haptic,  AW8671X_REG_DETCFG3, &reg_val, AW_I2C_BYTE_ONE);
	d2s_gain_default = reg_val & AW8671X_BIT_DETCFG3_D2S_GAIN;
	i2c_w_bits(aw_haptic, AW8671X_REG_DETCFG3,
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_CONTCFG6,
		   ...
		   AW8671X_BIT_CONTCFG6_TRACK_ENABLE);
	/* enable auto break */
	i2c_r_bytes(aw_haptic, AW8671X_REG_PLAYCFG3, &reg_val, AW_I2C_BYTE_ONE);
	...
	i2c_w_bytes(aw_haptic, AW8671X_REG_CONTCFG7, cont_config,
		    AW_I2C_BYTE_THREE);
	/* TRACK_MARGIN */
	if (!aw_haptic->info.cont_track_margin) {
		aw_err("awinic->info.cont_track_margin = 0!");
	} else {
		i2c_w_bytes(aw_haptic, AW8671X_REG_CONTCFG11,
			   &aw_haptic->info.cont_track_margin, AW_I2C_BYTE_ONE);
	}
	/* DRV_WIDTH */
	i2c_w_bytes(aw_haptic, AW8671X_REG_CONTCFG3,
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_DETCFG3,
		   AW8671X_BIT_DETCFG3_D2S_GAIN_MASK, d2s_gain_default);
...
static uint8_t aw8671x_rtp_get_fifo_afs(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_SYSST, &reg_val, AW_I2C_BYTE_ONE);
...
static uint8_t aw8671x_get_osc_status(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_SYSST2, &state, AW_I2C_BYTE_ONE);
...
static void aw8671x_get_lra_resistance(struct aw_haptic *aw_haptic)
{
	...
	/* ADC_FS */
	i2c_r_bytes(aw_haptic,  AW8671X_REG_DETCFG1, &reg_val[0],
		    AW_I2C_BYTE_ONE);
	adc_fs_default = reg_val[0] & AW8671X_BIT_DETCFG1_ADC_FS;
	i2c_w_bits(aw_haptic, AW8671X_REG_DETCFG1,
		   AW8671X_BIT_DETCFG1_ADC_FS_MASK,
		   AW8671X_BIT_DETCFG1_ADC_FS_12KHZ);
	/* D2S_GAIN*/
	i2c_r_bytes(aw_haptic,  AW8671X_REG_DETCFG3, &reg_val[0],
		    AW_I2C_BYTE_ONE);
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_DETRD1, reg_val, AW_I2C_BYTE_TWO);
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_DETCFG1,
		   AW8671X_BIT_DETCFG1_ADC_FS_MASK, adc_fs_default);
	i2c_w_bits(aw_haptic, AW8671X_REG_DETCFG3,
		   AW8671X_BIT_DETCFG3_D2S_GAIN_MASK, d2s_gain_default);
}
			       ...
			       char *buf)
{
	...
	for (i = 0; i < AW8671X_REG_RTPDATA; i++)
		i2c_r_bytes(aw_haptic, AW8671X_REG_RSTCFG, reg_array,
			    AW8671X_REG_RTPDATA);
	for (i = AW8671X_REG_RTPDATA + 1; i < AW8671X_REG_RAMDATA; i++)
		i2c_r_bytes(aw_haptic, (AW8671X_REG_RTPDATA + 1),
	...
	for (i = AW8671X_REG_RAMDATA + 1; i <= AW8671X_REG_ANACFG14; i++)
		i2c_r_bytes(aw_haptic, (AW8671X_REG_RAMDATA + 1),
				    ...
				    struct aw_haptic_container *awinic_cont)
{
	...
	i2c_w_bytes(aw_haptic, AW8671X_REG_RTPCFG1, rtp_addr, AW_I2C_BYTE_FIVE);
	i2c_r_bytes(aw_haptic, AW8671X_REG_RTPCFG3, reg_val, AW_I2C_BYTE_THREE);
	...
	while (i < awinic_cont->len) {
		...
		i2c_w_bytes(aw_haptic, AW8671X_REG_RAMDATA,
	...
	while (i < awinic_cont->len) {
		...
		i2c_r_bytes(aw_haptic, AW8671X_REG_RAMDATA, ram_data, len);
...
static uint64_t aw8671x_get_theory_time(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_SYSCTRL2, &reg_val, AW_I2C_BYTE_ONE);
			    ...
			    struct device_node *np)
{
	...
	val = of_property_read_u8(np, "aw8671x_cont_drv2_lvl",
				  &aw_haptic->info.cont_drv2_lvl);
	if (val != 0)
		aw_info("aw8671x_cont_drv2_lvl not found");
	...
	val = of_property_read_u8(np, "aw8671x_cont_drv_width",
				  &aw_haptic->info.cont_drv_width);
	if (val != 0)
		aw_info("aw8671x_cont_drv_width not found");
...
static void aw8671x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bytes(aw_haptic, AW8671X_REG_SYSCTRL4, reg_val, AW_I2C_BYTE_ONE);
	/* Get vmax */
	if (aw_haptic->info.bst_vol_default > 0)
		aw_haptic->vmax = aw_haptic->info.bst_vol_default;
	/* Get gain */
	i2c_r_bytes(aw_haptic, AW8671X_REG_PLAYCFG2, reg_val, AW_I2C_BYTE_ONE);
	aw_haptic->gain = reg_val[0];
	/* Get wave_seq */
	i2c_r_bytes(aw_haptic, AW8671X_REG_WAVCFG1, reg_val, AW_I2C_BYTE_EIGHT);
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_CONTCFG13,
	...
	i2c_w_bytes(aw_haptic, AW8671X_REG_CONTCFG10,
	...
	i2c_w_bits(aw_haptic, AW8671X_REG_CONTCFG5,
					   ...
					   const char *buf, size_t count)
{
	...
	if (rc < 0)
		return rc;
	i2c_w_bytes(aw_haptic, AW8671X_REG_CONTCFG4,
					  ...
					  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		i2c_r_bytes(aw_haptic, AW8671X_REG_CONTCFG6, reg_array,
			    AW_I2C_BYTE_ONE);
		...
		i2c_w_bytes(aw_haptic, AW8671X_REG_CONTCFG6, reg_array,
			    AW_I2C_BYTE_TWO);
	}
	return count;
}
					   ...
					   const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		i2c_w_bytes(aw_haptic, AW8671X_REG_CONTCFG8, reg_array,
			    AW_I2C_BYTE_TWO);
	}
	return count;
}
...
struct aw_haptic_func aw8671x_func_list = {
	...
	.juge_rtp_going = aw8671x_juge_rtp_going,
