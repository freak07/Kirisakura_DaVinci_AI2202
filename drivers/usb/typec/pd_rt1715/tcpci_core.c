...
int aura_screen_on = 0;
EXPORT_SYMBOL(aura_screen_on);
...
int tcpc_device_irq_enable(struct tcpc_device *tcpc)
{
	...
	if (!tcpc->ops->init) {
		printk("%s Please implment tcpc ops init function\n",
		__func__);
		return -EINVAL;
	}
...
MODULE_LICENSE("GPL");
MODULE_SOFTDEP("pre: pd_dbg_info");
