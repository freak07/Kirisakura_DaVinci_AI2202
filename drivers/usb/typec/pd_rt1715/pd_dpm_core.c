...
struct rt_charger_info *g_info = NULL;
EXPORT_SYMBOL(g_info);
//ASUS ---

uint16_t vid_ext = 0;
EXPORT_SYMBOL(vid_ext);
...
static int pps_request_thread_fn(void *param)
{
	...
	while (true) {
		ret = wait_event_interruptible(tcpc->pd_port.pps_request_wait_que,
				atomic_read(&tcpc->pd_port.pps_request) ||
				kthread_should_stop());
		...
		while (!wait_event_timeout(tcpc->pd_port.pps_request_wait_que,
					!atomic_read(&tcpc->pd_port.pps_request) ||
					kthread_should_stop(),
...
{
	uint32_t snk_pdo = APDO_PPS(
				pd_port->request_v_apdo, pd_port->request_v_apdo, pd_port->request_i_apdo, 0);
	DPM_DBG("Request, v_apdo=%d, i_apdo=%d\r\n", pd_port->request_v_apdo, pd_port->request_i_apdo);
...
//ASUS +++
		...
		DPM_INFO("asus not support miss match flag\r\n");
//ASUS ---
	}
		...
		struct tcp_dpm_pd_request *pd_req)
{
	...
	uint8_t charging_policy = pd_port->dpm_charging_policy;
	struct pd_port_power_caps *src_cap = &pd_port->pe_data.remote_src_cap;
	uint32_t snk_pdo = APDO_PPS(pd_req->mv, pd_req->mv, pd_req->ma, 0);
...
//ASUS +++
	if (g_info != NULL && (charging_policy & DPM_CHARGING_POLICY_MASK)!= DPM_CHARGING_POLICY_PPS) {
...
static int dpm_vdm_ufp_response_id(struct pd_port *pd_port)
{
	...
	return pd_reply_svdm_request(pd_port, CMDT_RSP_ACK,
		pd_check_rev30(pd_port) ? pd_port->id_vdo_nr : 3,  pd_port->id_vdos);
}
