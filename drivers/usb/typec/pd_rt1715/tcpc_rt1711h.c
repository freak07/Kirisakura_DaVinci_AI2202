...
#define RT1711H_DRV_VERSION	"2.0.1_G"
...
static int rt1711_init_alert(struct tcpc_device *tcpc)
{
	...
	struct pinctrl *rt1711_pinctrl;
	struct pinctrl_state *rt1711_pinctrl_state_active;
	int rc=0;
	int ret;
	char *name;
	int len;
	...
	printk("%s: Set rt1711 pinctrl state\n", __func__);
	rt1711_pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR(rt1711_pinctrl)){
		printk("%s: failed at rt1711 devm_pinctrl_get\n", __func__);
		return PTR_ERR(rt1711_pinctrl);
	}
	rt1711_pinctrl_state_active = pinctrl_lookup_state(rt1711_pinctrl, "rt1715_int_active");
	if (IS_ERR(rt1711_pinctrl_state_active)){
		printk("%s: failed at rt1711 pinctrl_lookup_state\n", __func__);
		return PTR_ERR(rt1711_pinctrl_state_active);
	}
	rc = pinctrl_select_state(rt1711_pinctrl, rt1711_pinctrl_state_active);
	if(rc < 0)
		printk("%s: failed to set rt1711 pinctrl state\n", __func__);
	printk("%s: Set rt1711 pinctrl state over\n", __func__);
...
static int rt_parse_dt(struct rt1711_chip *chip, struct device *dev)
{
	...
	ret = of_get_named_gpio(np, "rt1711,irq_pin", 0);
	if (ret < 0) {
				...
				const struct i2c_device_id *id)
{
	...
	if (i2c_check_functionality(client->adapter,
		...
		printk("I2C functionality : OK...\n");
	else
		printk("I2C functionality check : failuare...\n");
	...
	chip->irq_wake_lock =
		wakeup_source_register(NULL, "rt1711h_irq_wakelock");

	chip->chip_id = chip_id;
	printk("rt1711h_chipID = 0x%0x\n", chip_id);
...
static int rt1711_i2c_prepare(struct device *dev)
{
...
static int __init rt1711_init(void)
{
	...
	int ret;
	printk("rt1711h_init (%s): initializing...\n", RT1711H_DRV_VERSION);
...
MODULE_SOFTDEP("pre: tcpc_class");

/**** Release Note ****
 * 2.0.1_G
 *	First released PD3.0 Driver on General platform
 */
